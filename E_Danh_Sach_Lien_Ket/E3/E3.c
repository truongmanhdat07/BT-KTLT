...
void nhapdt(Node *dt[],int);
void indt(Node *dt[],int);
float tinhgt(Node *dt[],int);
float tinhdh(Node *dt[],int);
void congdt(Node *dt[],int);
void giaiphong(Node *dt[],int);

int main(){
	int sodt;
	printf("-----------------------------------------------------\n");
	printf("\nChon so da thuc muon nhap : ");   scanf("%d",&sodt);
...
void nhapdt(Node *dt[],int a){
	int bac;
	do{
		printf("\n\tBac da thuc %d  = ",a+1);     scanf("%d",&bac);
		if (bac<0)  printf("\nLoi : Bac cua tha thuc phai lon hon hoac bang 0! Vui long nhap lai!\n");
	}while(bac<0);
    Node* dau  = NULL;
    Node* duoi = NULL;
	for (int j=bac;j>=0;j--){
...
float tinhgt(Node *dt[],int a){
	...
	float x,gt=0;
	do{
		printf("\nChon da thuc muon tinh : ");    scanf("%d",&b);
...
void congdt(Node* dt[],int a){
	int b[2];
	do{
		printf("\nChon 2 da thuc muon cong : ");		scanf("%d %d",&b[0],&b[1]);
		if (1>b[0]||b[0]>a||1>b[1]||b[1]>a) printf("\nLoi : Da thuc ko ton tai hay chon lai!\n");
	}while(1>b[0]||b[0]>a||1>b[1]||b[1]>a);
	Node* p1 = dt[b[0] - 1];
    Node* p2 = dt[b[1] - 1]; 
